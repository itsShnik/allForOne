...
def gru_unit(__C):
    return nn.GRU(input_size=__C.WORD_EMBED_SIZE, hidden_size=__C.HIDDEN_SIZE, num_layers=1, batch_first=True).cuda()
...
class AttentionTeams(nn.Module):
    def __init__(self, __C):
        ...
        self.unit_list = [gru_unit(__C) for _ in range(__C.NUM_TEAMS)]

        # initialize a lead unit
        self.lead_unit = gru_unit(__C)
    ...
    def forward(self, lang_feat, img_feat):
        ...
        feat = torch.cat((lang_feat, img_feat), -2)
